...
class CurriculumLoader:
            ...
            for json_file in json_path.glob("*.json"):
                try:
                    ...
                    if curriculum_data:
                        ...
                        if subject_name.lower() in ['cografya', 'felsefe'] and len(topics) > 0:
    ...
    def _find_subject_key(self, yks_data: Dict, subject: str) -> Optional[str]:
        ...
        subject_mapping = {
            ...
            'i̇nkilap ve ataturkculuk': ['inkilap_ve_ataturkculuk'],
            'matematik': ['matematik'],
            'fizik': ['fizik'],
            'kimya': ['kimya'],
            'biyoloji': ['biyoloji'],
        ...
        for grade, grade_data in subject_data.items():
            if isinstance(grade_data, dict):
                ...
                if grade in ['A', 'B', 'C']:
                    ...
                    grade_label = f"Duzey {grade}"
                else:
                    grade_label = f"Sinif {grade}"
    ...
    def _process_standard_grade_structure(self, subject_data: Dict, subject: str) -> List[Dict]:
        """Process standard grade structure for Cografya and Felsefe"""
        topics = []
                    ...
                    if isinstance(unit_content, dict):
                        ...
                        for code, objective_data in unit_content.items():
                            if isinstance(objective_data, dict) and 'baslik' in objective_data:
                                ...
                                if topic['unit']:
                                    content_parts.append(f"Unite: {topic['unit']}")
    ...
    def _process_letter_based_structure(self, subject_data: Dict, subject: str) -> List[Dict]:
        """Process letter-based structure for Turk Dili ve Edebiyati"""
        topics = []
        ...
        for letter, letter_data in subject_data.items():
            if isinstance(letter_data, dict):
                if 'alt' in letter_data:
                    ...
                    topics.extend(self._process_curriculum_level(
                        letter_data['alt'], subject, f"Bolum {letter}"
                    ))
                else:
                    # Direct structure
                    topics.extend(self._process_curriculum_level(
                        {letter: letter_data}, subject, f"Bolum {letter}"
                    ))
