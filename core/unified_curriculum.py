...
class UnifiedCurriculumSystem:
    ...
    def show_curriculum_browser(self, subject: str = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            if not subject:
                ...
                console.print(Panel("[bold cyan]📚 MUFREDAT TARAYICiSi[/bold cyan]", title="Unified Curriculum"))
                ...
                if not subjects:
                    console.print("[red]Hic mufredat yuklenmemis![/red]")
                    return None
    ...
    def _show_subject_details(self, subject: str) -> Dict[str, Any]:
        ...
        try:
            ...
            console.print(f"\n[bold cyan]📖 {subject} MUFREDATi[/bold cyan]")
            
            nav_summary = curriculum_data.get("navigation_summary", {})
            
            # Subject stats table
            stats_table = Table(title="Mufredat Istatistikleri")
            stats_table.add_column("Ozellik", style="cyan")
            stats_table.add_column("Deger", style="green")
            
            stats_table.add_row("Toplam Konu", str(curriculum_data.get("total_topics", 0)))
            stats_table.add_row("Sinif Sayisi", str(nav_summary.get("total_grades", 0)))
            stats_table.add_row("Unite Sayisi", str(nav_summary.get("total_units", 0)))
            stats_table.add_row("Ortalama Konu/Sinif", str(nav_summary.get("avg_topics_per_grade", 0)))
