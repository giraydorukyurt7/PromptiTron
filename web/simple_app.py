...
SAMPLE_CURRICULUM = {
    "Matematik": {
        "topics": [
            {"name": "Fonksiyonlar", "description": "Fonksiyon kavramı ve özellikleri"},
            {"name": "Türev", "description": "Türev kavramı ve uygulamaları"},
            {"name": "İntegral", "description": "İntegral kavramı ve uygulamaları"},
            {"name": "Geometri", "description": "Düzlem ve uzay geometrisi"}
        ]
    },
    "Fizik": {
        "topics": [
            {"name": "Hareket", "description": "Kinematik ve dinamik"},
            {"name": "Enerji", "description": "Enerji türleri ve korunumu"},
            {"name": "Elektrik", "description": "Elektrik ve manyetizma"},
            {"name": "Dalgalar", "description": "Dalga hareketi ve özellikleri"}
        ]
    },
    "Kimya": {
        "topics": [
            {"name": "Atom Yapısı", "description": "Atom modelleri ve yapısı"},
            {"name": "Periyodik Sistem", "description": "Elementlerin özellikleri"},
            {"name": "Kimyasal Bağlar", "description": "Bağ türleri ve özellikleri"},
            {"name": "Reaksiyonlar", "description": "Kimyasal reaksiyonlar"}
        ]
    },
    "Biyoloji": {
        "topics": [
            {"name": "Hücre", "description": "Hücre yapısı ve işlevleri"},
            {"name": "Genetik", "description": "Kalıtım ve genetik"},
            {"name": "Evrim", "description": "Evrim teorisi ve kanıtları"},
            {"name": "Ekoloji", "description": "Ekosistem ve çevre"}
        ]
    }
}
    ...
    async def call_api(self, endpoint: str, method: str = "POST", data: dict = None) -> dict:
        ...
        try:
            ...
            if endpoint == "/chat":
                return {
                    "success": True,
                    "response": f"Bu bir test yanıtıdır. Mesajınız: {data.get('message', 'Mesaj bulunamadı')}",
                ...
                return {
                    ...
                    "questions": [
                        {
                            "question_id": "test_1",
                            "question_text": f"{data.get('subject', 'Test')} - {data.get('topic', 'Test konusu')} hakkında test sorusu",
                            "options": [
                                {"option_letter": "A", "option_text": "Seçenek A"},
                                {"option_letter": "B", "option_text": "Seçenek B"},
                                {"option_letter": "C", "option_text": "Seçenek C"},
                                {"option_letter": "D", "option_text": "Seçenek D"},
                                {"option_letter": "E", "option_text": "Seçenek E"}
                            ],
                            "correct_answer": "A",
                            "explanation": "Bu test açıklamasıdır."
                        }
                    ]
                }
                ...
                return {
                    ...
                    "study_plan": {
                        "plan_id": "test_plan",
                        "overall_strategy": "Sistematik çalışma planı test versiyonu",
                        "weekly_plans": [
                            "Hafta 1: Matematik temel konular",
                            "Hafta 2: Fizik hareket konuları",
                            "Hafta 3: Kimya atom yapısı"
                        ]
                    }
                }
                ...
                return {
                    ...
                    "results": [
                        {
                            "title": "Test Sonucu",
                            "content": f"'{data.get('query', 'test')}' araması için test sonucu",
                ...
                return {
                    "success": True,
                    "analysis": f"İçerik analizi: {data.get('content', 'Test içeriği')[:100]}... Bu içerik test analizi sonucudur."
                }
            else:
...
def ai_assistant():
    ...
    try:
        ...
        result = {
            "success": True,
            "response": f"AI Asistan Test Yanıtı: {user_input} mesajına yanıt olarak, bu sistem test modunda çalışmaktadır. Gerçek AI entegrasyonu için backend servislerinin çalışması gerekiyor.",
...
def export_data(format):
    ...
    try:
        ...
        if format == 'json':
            ...
            content = "# Promptitron Konuşma Geçmişi (Test)\n\n"
            ...
            story.append(Paragraph("Promptitron Konuşma Geçmişi (Test)", styles['Title']))
            story.append(Spacer(1, 20))
            
            for item in ui_manager.conversation_history:
                if item['type'] == 'ai_assistant':
                    story.append(Paragraph(f"Kullanıcı: {item['user']}", styles['Normal']))
